...
export default function generateIconsAndMetadata(baseUrl) {
  return new Promise((resolve, reject) => {
    ...
    const config = {
      ...
      path: `${baseUrl}/assets`,      // Path for overriding default icons path. `string`
      url: `${baseUrl}/assets`,       // Absolute URL for OpenGraph image. `string`
    ...
    favicons(sourceImage, config, (error, response)=> {
      if (error) {
        ...
        fs.mkdirpSync(distAssetsDir)
        Promise.all([
          saveIcons(distAssetsDir, response.images),
          saveFiles(distAssetsDir, response.files),
          saveHtmlIconsMetadata(distAssetsDir, response.html),
        ]).then(resolve)
      }
    })
  })
